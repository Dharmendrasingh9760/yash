...
@section('content')
    ...
    <div class="container mx-auto py-10 px-6">
        <div class="bg-gray-50 border border-gray-200 rounded-lg shadow-sm p-6 space-y-6">
                ...
                @if ($water->payment_status == 1)
                    @if ($water->remaining === '0')
                        <div class="flex gap-3 items-center ">
                            <button onclick="printPDF({{ $water->id }})"
                            ...
                            <a href="{{ route('water.waterPdf', ['id' => $water->id]) }}"
            ...
            <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6 space-y-6">
                <h3 class="text-xl font-semibold mb-4">Water Tax Details</h3>

                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div>
                        <p class="text-gray-600">Emitra Code</p>
                        <p class="text-lg font-medium">{{ $water->emitra_12_code ?? 'NA' }}</p>
                    </div>
                    <div>
                        <p class="text-gray-600">Mobile Number</p>
                        <p class="text-lg font-medium">{{ $water->mobile ?? 'NA' }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div>
                        ...
                        <p class="text-lg font-medium">{{ $water->email ?? 'NA' }}</p>
                    </div>
                    {{-- <div>
                        <p class="text-gray-600">Father's name</p>
                        <p class="text-lg font-medium">{{ $water->name_of_father ?? 'NA' }}</p>
                    </div> --}}
                </div>
            </div>
            ...
            @if ($water->status == 2)
                <div class="bg-yellow-50 border border-yellow-400 rounded-lg shadow-sm p-6 mt-6">
                    ...
                    <p class="text-lg font-medium">{{ $water->remark ?? 'NA' }}</p>
                </div>
            @endif
            @if ($water->status == 1)
                <div
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-xl font-medium text-gray-700">
                            {{ '₹' . ' ' . ($water->remaining != 0 ? $water->remaining : $water->amount) }}
                        </p>
                    </div>
                    ...
                    @elseif ($water->payment_status == 1 && $water->remaining === '0')
                        <button
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const maxAmount = {{ $water->remaining }};
    ...
    <script>
        function printPDF(id) {
            ...
            iframe.src = '{{ route('waterPrintPdf.print.pdf', ':id') }}'.replace(':id', id);
