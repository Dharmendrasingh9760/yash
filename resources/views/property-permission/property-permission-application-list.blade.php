...
<div class="xl:px-[4rem] px-[10px] py-[1rem]">
    ...
    @else
        ...
        <div
            ...
            <div>
                <div class="font-semibold lg:text-[18px] text-[16px] leading-[22px] text-[#000000]">{{ $propertyData['id'] ?? "NA"}} | {{ $propertyData['name_chairman_owner'] ?? "NA"}}</div>
            ...
            @elseif($propertyData->status == 1)
                <p class=" font-medium lg:text-[18px]  text-[16px] text-[#F26F00]  leading-[22px] " style="color:green;">Approved</p>
                @if($propertyData->amount)
                <p class=" font-medium lg:text-[18px]  text-[16px]  leading-[22px] ">{{ $propertyData->payment_status == 0 ? 'Payment Pending - ' . $propertyData->amount : 'Payment Successful - ' . $propertyData->amount }}
                <i class="fa-solid fa-indian-rupee-sign"></i></p>
                {{$propertyData->chalan_number}}
                @endif
            @else
                <p class=" font-medium lg:text-[18px]  text-[16px] text-[#F26F00]  leading-[22px] " style="color:red;">Rejected</p>
            @endif
            </div>
            </div>
            <a href="{{route('property.permission.application.nonresidential.view',['id' => $propertyData['id']])}}"
    ...
    @else
        ...
        <div
            ...
            <a href="{{route('property.permission.application.residential.view',['id' => $propertyData['id']])}}"
                class="font-medium lg:text-[20px] text-[18px] leading-[25px] text-[#202224] cursor-pointer">
                View Details→
            </a>
        </div>
