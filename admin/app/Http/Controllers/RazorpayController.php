...
class RazorpayController extends Controller
{
    ...
    public function createOrder(Request $request)
    {
        ...
        try {
            ...
            return response()->json(['order_id' => $order['id'], 'amount' => $order['amount'], 'type' => $type, 'id' => $id, 'type_tax' => $type_tax]);
        } catch (\Exception $e) {
    ...
    public function verifyPayment(Request $request)
    {
        ...
        if ($type === "PROPERTY") {
            ...
            if ($property) {
                ...
                $property->amount = 0;
                $type_tax = $request->input('data.type_tax');
                if (in_array('house', $type_tax) && in_array('water', $type_tax)) {
